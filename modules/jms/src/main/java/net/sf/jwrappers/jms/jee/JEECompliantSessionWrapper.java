...
public class JEECompliantSessionWrapper extends SessionWrapper {
    private static final Log log = LogFactory.getLog(JEECompliantSessionWrapper.class);
    
    private final boolean warnOnly;

    public JEECompliantSessionWrapper(boolean warnOnly) {
    ...
    @Override
    public void run() {
        String msg = "run() not allowed in a JEE environment";
        if (warnOnly) {
            log.warn(msg);
            super.run();
        } else {
